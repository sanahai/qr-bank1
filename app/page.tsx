...
import { 
  ...
  MapPin, CheckCircle, Copy, ExternalLink, X, MousePointerClick 
} from "lucide-react";
...
return (
    <div className="min-h-screen bg-white">
      ...
      <header className="px-6 pt-10 pb-16 text-center max-w-4xl mx-auto">
        ...
        {/* 버튼 수정: 하나로 통합하고 크기 키움 */}
        <div className="flex flex-col items-center justify-center">
          <Link 
            href="#preview" 
            className="w-full md:w-auto px-10 py-4 bg-blue-600 text-white rounded-2xl font-bold text-xl hover:bg-blue-700 transition-all shadow-lg shadow-blue-200 flex items-center justify-center gap-2 transform hover:scale-105"
          >
            <MousePointerClick size={24} /> 서비스 체험하기
          </Link>
          <p className="text-xs text-gray-400 mt-3">👆 클릭해서 실제 작동 화면을 확인해보세요</p>
        </div>
      </header>
      ...
      <section id="preview" className="px-6 py-12 text-center overflow-hidden">
        ...
        <div 
          ...
          className="relative mx-auto w-56 h-[450px] bg-gray-900 rounded-[2.5rem] border-8 border-gray-900 shadow-2xl cursor-pointer hover:scale-105 transition-transform duration-300 group"
        >
          {/* 노치 */}
          ...
          <div className="w-full h-full bg-white rounded-[2rem] overflow-hidden relative flex flex-col pt-8 px-4 items-center">
            ...
            <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 z-30 flex items-center justify-center transition-colors">
              <div className="bg-white/90 backdrop-blur px-4 py-2 rounded-full text-sm font-bold shadow-lg opacity-0 group-hover:opacity-100 transition-opacity transform translate-y-2 group-hover:translate-y-0 text-blue-600 flex items-center gap-1">
                <MousePointerClick size={16}/> 눌러서 체험하기
              </div>
            </div>
            ...
            <div className="w-full grid grid-cols-4 gap-2 opacity-50">
               {[1,2,3,4,5,6,7,8].map(i => <div key={i} className="aspect-square bg-gray-100 rounded-md"></div>)}
            </div>
          </div>
        </div>
      </section>
