...
export default function Home() {
  ...
  // 📝 신청서 제출 함수 (수정됨: 연락처 필수 체크)
  const handleSubmit = async () => {
    // 1. 필수값 체크 (연락처 포함)
    if (!applyForm.shop_name || !applyForm.owner_name || !applyForm.bank_account || !applyForm.contact) {
      alert("매장명, 대표자명, 계좌번호, 연락처는 필수 입력 항목입니다.");
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-white">
      <nav className="flex items-center justify-between px-6 py-3 border-b bg-white/80 backdrop-blur-sm sticky top-0 z-40">
        <Link href="/" className="text-xl font-black tracking-tighter hover:opacity-70 transition-opacity cursor-pointer">QR BANK</Link>
        <div><Link href="/admin" className="text-sm font-bold text-gray-500 hover:text-black py-2 px-2 transition-colors">운영자 로그인</Link></div>
      </nav>

      <header className="px-6 pt-10 pb-16 text-center max-w-4xl mx-auto">
        <div className="inline-block px-3 py-1 bg-gray-100 text-gray-600 rounded-full text-xs font-bold mb-4">사장님을 위한 간편 결제 솔루션</div>
        <h1 className="text-4xl md:text-6xl font-black text-gray-900 leading-tight mb-6">계좌번호 묻지마세요.<br /><span className="text-blue-600">QR로 바로 입금</span>받으세요.</h1>
        <p className="text-base md:text-lg text-gray-600 mb-8 max-w-2xl mx-auto leading-relaxed">카운터에 QR코드 하나만 붙여두세요.<br className="md:hidden"/>고객은 카메라로 찍고, 앱으로 바로 송금합니다.<br />수수료 0원, 가입비 0원. 지금 바로 시작하세요.</p>
        
        <div className="flex flex-col md:flex-row items-center justify-center gap-3">
          <Link href="#preview" className="w-full md:w-auto px-8 py-4 bg-gray-100 text-gray-700 rounded-2xl font-bold text-lg hover:bg-gray-200 transition-all flex items-center justify-center gap-2">
            <MousePointerClick size={20} /> 서비스 체험하기
          </Link>
          <button onClick={() => setIsApplyOpen(true)} className="w-full md:w-auto px-8 py-4 bg-blue-600 text-white rounded-2xl font-bold text-lg hover:bg-blue-700 transition-all shadow-lg shadow-blue-200 flex items-center justify-center gap-2">
      ...
      <section className="px-6 py-10 bg-gray-50">
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100"><div className="w-10 h-10 bg-blue-100 rounded-xl flex items-center justify-center text-blue-600 mb-4"><Zap size={20} fill="currentColor" /></div><h3 className="text-lg font-bold mb-2">1초 연결</h3><p className="text-sm text-gray-500">고객이 QR을 찍으면<br/>은행 앱이 자동 실행됩니다.</p></div>
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100"><div className="w-10 h-10 bg-green-100 rounded-xl flex items-center justify-center text-green-600 mb-4"><ShieldCheck size={20} /></div><h3 className="text-lg font-bold mb-2">안전한 계좌</h3><p className="text-sm text-gray-500">실명 인증된 계좌번호만<br/>안전하게 전달됩니다.</p></div>
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100"><div className="w-10 h-10 bg-purple-100 rounded-xl flex items-center justify-center text-purple-600 mb-4"><Smartphone size={20} /></div><h3 className="text-lg font-bold mb-2">간편한 관리</h3><p className="text-sm text-gray-500">복잡한 단말기 필요 없이<br/>스마트폰 하나로 해결됩니다.</p></div>
        </div>
      </section>
      ...
      <section id="preview" className="px-6 py-12 text-center overflow-hidden">
        ...
        <div onClick={() => setIsSampleOpen(true)} className="relative mx-auto w-56 h-[450px] bg-gray-900 rounded-[2.5rem] border-8 border-gray-900 shadow-2xl cursor-pointer hover:scale-105 transition-transform duration-300 group">
          ...
          <div className="w-full h-full bg-white rounded-[2rem] overflow-hidden relative flex flex-col pt-8 px-4 items-center">
            <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 z-30 flex items-center justify-center transition-colors"><div className="bg-white/90 backdrop-blur px-4 py-2 rounded-full text-sm font-bold shadow-lg opacity-0 group-hover:opacity-100 transition-opacity transform translate-y-2 group-hover:translate-y-0 text-blue-600 flex items-center gap-1"><MousePointerClick size={16}/> 눌러서 체험하기</div></div>
            <div className="text-[10px] text-gray-400 mb-2 flex items-center gap-1"><MapPin size={10}/> 매장 정보</div><div className="font-bold text-base mb-4 text-gray-800">황금 붕어빵</div><div className="w-full bg-blue-50 rounded-xl p-3 mb-3 border border-blue-100"><div className="text-[9px] text-blue-500 font-bold mb-1">송금 계좌</div><div className="font-black text-lg text-gray-900 mb-1">3333-01...</div><div className="w-full bg-blue-600 h-6 rounded-lg"></div></div><div className="w-full grid grid-cols-4 gap-2 opacity-50">{[1,2,3,4,5,6,7,8].map(i => <div key={i} className="aspect-square bg-gray-100 rounded-md"></div>)}</div>
          </div>
        </div>
      </section>
      ...
      <footer className="bg-gray-900 text-gray-400 py-8 px-6 text-center text-xs">
        ...
        <div className="space-y-1"><p>사업자등록번호 : 123-45-67890 | 대표 : 홍길동</p><p>서울특별시 강남구 테헤란로 123</p></div><p className="mt-6 opacity-50">© 2026 QR BANK. All rights reserved.</p>
      </footer>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl p-6 relative animate-in zoom-in duration-200">
            ...
            <div className="space-y-3">
              <div><label className="block text-xs font-bold text-gray-500 mb-1">매장 이름 <span className="text-red-500">*</span></label><input className="w-full border p-3 rounded-lg text-black bg-gray-50" value={applyForm.shop_name} onChange={e=>setApplyForm({...applyForm, shop_name: e.target.value})} placeholder="예: 황금 붕어빵" /></div>
              <div><label className="block text-xs font-bold text-gray-500 mb-1">대표자 성함 <span className="text-red-500">*</span></label><input className="w-full border p-3 rounded-lg text-black bg-gray-50" value={applyForm.owner_name} onChange={e=>setApplyForm({...applyForm, owner_name: e.target.value})} placeholder="예: 홍길동" /></div>
              <div>
                <label className="block text-xs font-bold text-gray-500 mb-1">입금받을 계좌 <span className="text-red-500">*</span></label>
              ...
              <div>
                ...
                <input className="w-full border p-3 rounded-lg text-black bg-gray-50" value={applyForm.contact} onChange={e=>setApplyForm({...applyForm, contact: e.target.value})} placeholder="010-0000-0000" />
              </div>
            </div>

            <button onClick={handleSubmit} className="w-full mt-6 bg-blue-600 text-white font-bold py-4 rounded-xl hover:bg-blue-700 shadow-lg">신청서 제출하기</button>
          </div>
        </div>
      )}
